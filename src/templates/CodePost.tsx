					...
					renderToken={(token, tokenProps) => {
						switch (token.type) {
							...
							case TypeScriptTokenType.Diagnostic:
								return <TypeScriptDiagnosticToken message={token.diagnosticMessage} {...tokenProps} />;
							default:
...
const subtitleStyle = css([type.subtilte]);
		...
		const editableContext: EditableContext = {
			...
			onStartEditing: async (fileName) => {
				if (!initializedFiles[fileName]) {
					...
					if (!initializedTsEnv) {
						const [{ createVirtualTypeScriptEnvironment }, { createSystem }, { lib }, { getExtraLibFiles }] =
							await Promise.all([
						...
						const sysFiles = new Map([...Array.from(lib.core.entries()), ...Array.from(extraFiles)]);
	...
	return (
		<Layout>
			...
			<div>
				...
				{post.frontmatter.subtitle ? <h2 css={subtitleStyle}>{post.frontmatter.subtitle}</h2> : null}
