...
module.exports = (
  ...
  visitWithParents(markdownAST, [`html`, `jsx`], (node, ancestors) => {
    ...
    const customOptions = getImageOptions(node, ancestors)
    rawHtmlNodes.push({ node, inLink, customOptions })
  })
  ...
  visitWithParents(
    ...
    (node, ancestors) => {
      const inLink = ancestors.some(findParentLinks)
      const customOptions = getImageOptions(node, ancestors)
      markdownImageNodes.push({ node, inLink, customOptions })
    }
  )
  ...
  const generateImagesAndUpdateNode = async function(
    ...
    const options = _.defaults(customOptions, globalOptions)
    ...
    let rawHTML = `
  <span
    class="${[imageWrapperClass, options.wrapperClassName].join(' ')}"
...
function getPrevSibling(node, ancestors) {
  ...
  if (parent) {
    ...
    return getPrevSibling(parent, ancestors)
  }
}

function getImageOptions(imgNode, ancestors) {
  const candidate = getPrevSibling(imgNode, ancestors)
  if (candidate && candidate.type === `html` && candidate.value.startsWith('<!--@@')) {
    const yamlRaw = candidate.value.match(/^<!--@@(.*?)[\r\n]+([\s\S]*)-->$/)[2]
    if (yamlRaw) {
