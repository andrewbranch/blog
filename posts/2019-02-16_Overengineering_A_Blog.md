...
- able to reference outside libraries and arbitrary other code not shown in the code block.

They were expressly _not_ intended to be:

- a good code editor,
- so complex that I never finish and publish this blog.

I knew I wasn’t setting out to build an editor. I was building a learning tool, 90% of whose value is not in editing—the type info and diagnostics tooltips were far more important than the ability to edit. So I think choosing not to use Monaco was a reasonable decision after all. Having language support and great syntax highlighting, which Monaco could have provided better than I could do myself, was crucially important, but pushing a full-fledged editor powerful enough to power the code editor [used by 2.6 million people](https://code.visualstudio.com/blogs/2017/11/16/connect) onto a bunch of people who came to _read an article_ who are almost certainly not going to edit the code samples seems like an irresponsible use of your bandwidth.
...
My real code is a lot more complex, but mostly because of performance hacks. This is a good representation of the actual architecture.

The biggest factor that makes this example an oversimplification is this: because in TypeScript, changing something on line 1 can change the state of something on line 10, I need to analyze the full text, every line, on every change. But if I use the new program state to build a new list of Decorations for the whole code sample, then every node (basically a React component for every word and punctuation mark) would re-render on every change. My solution involves a lot of caching, hashing, and memoization. I analyze the whole document on every change (well, that’s an oversimplification too—I’ll get there), then break the result down into tokens by line, and cache that. If I can tell that a whole line of new tokens are identical to a whole line of existing tokens by comparing their hashes, I can just return the existing Decoration objects. Slate will understand this as a directive that those lines don’t need to re-render, which removes a noticeable amount of latency on each keystroke.

## Static Rendering
I’m telling this piece of the story a little out of order, because I think it’s important context for everything that follows. I didn’t initially plan for this, but as I began to realized how much library code would be necessary to achieve editable code blocks with language support (the TypeScript compiler is larger than the rest of my blog’s assets combined), I once again felt a moral imperative to respect your data usage. Generally speaking, people probably don’t expect to have to download a compiler in order to read a blog post.

I also realized that none of the really heavy pieces are necessary until the moment a reader starts _editing_ a code sample. I could analyze the initial code text at build time, attach syntax highlighting information, type information, and compiler diagnostics as static data, and reference that data during render, all for a fraction of the weight of the compiler and syntax highlighter itself.

[Gatsby](https://www.gatsbyjs.org) made this fairly easy. During a build, I search the Markdown posts for TypeScript code blocks, run the static analysis on their contents, and inject the results into the [page context](https://www.gatsbyjs.org/docs/creating-and-modifying-pages/#pass-context-to-pages) (which becomes available as a prop in the page template component).
...
### A Compiler Host for the Browser
...
1. `getSyntacticClassifications` provides a sort of high-level classification of tokens (e.g. `className`, `jsxOpenTagName`, `identifier`), from which I pick the name-y ones to be candidates for “quick info” (the tooltip contents);
2. `getSyntacticDiagnostics`, which tells me the location and nature of a syntactic error, like putting a curly brace where it doesn’t belong; and finally,
3. `getSemanticDiagnostics`, which tells me the location and nature of type errors like `Object is possibly undefined` or `Cannot find name 'Recat'`.
...
## Syntax Highlighting
### First attempt: Prism
Here’s the sticky part.  The first tokenizer I tried for syntax highlighting was [Prism](https://prismjs.com), and it didn’t cut it. Prism may produce perfect results for simple grammars, but complex TypeScript just didn’t look right. (Part of this is due to Prism’s inherent limitations; part is due to its rather sparse [grammar definition](https://github.com/PrismJS/prism/blob/11695629f12925c586702453beaee5f4825d0ebd/components/prism-typescript.js). Other grammars are more complete.) By no means do I intend to disparage anyone else’s work. On the contrary, I knew that Prism doesn’t claim to be perfect, but rather aims to be light, fast, and good enough—I didn’t realize just how _much_ lighter and faster it is for its tradeoffs until seeking a higher fidelity alternative.

### Second attempt: TypeScript
The obvious choice for perfect syntax highlighting, I thought, was the TypeScript compiler itself. I would already have access to it given what I was doing with the language service, and clearly it understands every character of the code in a markedly deeper, more semantic way than any regex engine. With some help from [a tweet](https://twitter.com/drosenwasser/status/1087744826156638208), I found `getSyntacticClassifications`, which did overall a little better than Prism on its own. Adding extra information from `getSemanticClassifications` improved things further.

One quirk of this approach was that referenced interface names and class names would only be colored as such if they actually existed. For instance, in the following code:

```ts
interface FirstInterfaaaaec {}
interface SecondInterface extends FirstInterface {}
```

In this early iteration of my syntax highlighting, the egregious typo `FirstInterfaaaec` would have caused `FirstInterface` to be a different color (the default identifier color, same as variables). Fixing the typo in line 1 would instantly change the color appearing in line 2. It was kind of interesting, but not necessarily desirable. A little distracting.
...
### Third and final attempt: TextMate Grammar
